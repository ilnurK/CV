...
SettingsDialog::SettingsDialog(settings_t *settings, QWidget *parent)
  ...
  display_method = new QComboBox(this);
  display_method->addItems({"отсутствуют", "     круги", "  квадраты"});
  display_method->setCurrentIndex(settings->display_method);
  display_method->setStyleSheet(
  ...
  colorVerticesButton->setStyleSheet(
      QString("background-color: %1; border: 1px solid #424242;")
          .arg(settings->color_vertices.name()));
  colorVerticesButton->setAccessibleName(settings->color_vertices.name());
  ...
  colorEdgesButton->setStyleSheet(
      QString("background-color: %1; border: 1px solid #424242;")
          .arg(settings->color_edges.name()));
  colorEdgesButton->setAccessibleName(settings->color_vertices.name());
  ...
  layout->addWidget(display_method, 1, 0, 1, 2);
...
void SettingsDialog::selectColor() {
  ...
  QColor selectedColor = color_dialog.getColor();
...
void SettingsDialog::applySettings(settings_t *settings) {
  settings->display_method = display_method->currentIndex();
  settings->type = type->currentIndex();
  settings->size = size->value();
  settings->thickness = thickness->value();
  settings->color_vertices = colorVerticesButton->accessibleName();
  settings->color_edges = colorEdgesButton->accessibleName();
  accept();
}
