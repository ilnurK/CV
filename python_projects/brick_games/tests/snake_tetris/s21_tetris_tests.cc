...
TEST(S21TetrisTestController, TestSimpleInit1) {
  ...
  S21TetrisCController *tetris_ctrl = InitTetrisController(&error);
  ...
  if (!error) {
    ...
    EXPECT_EQ(1, static_cast<int>(GetTetrisLevel(tetris_ctrl)));
  } else {
...
TEST(S21TetrisTestController, TestHighScore) {
  ...
  if (error == NULL_ERROR) {
    ...
    S21XYPair pair = GetExtraInfoAt(tetris_ctrl, 3);
    EXPECT_TRUE(pair.x < FIELD_WIDTH);
    EXPECT_TRUE(pair.y < FIELD_HEIGHT);

    size_t hs = 400;
    SetTetrisScore(tetris_ctrl, hs);
    WriteTetrisHighScore(tetris_ctrl, &error);
    bool is_init_error = false;
    size_t rhs = S21InitHighScore(tetris_log_file_name, &is_init_error);
    if (!error && !is_init_error) {
      EXPECT_EQ(hs, rhs);
...
TEST_F(S21TetrisFSMTest1, TestTouch1) {
  ...
  try {
    ...
    SetFSMState(fsm_, MOVING, NULL_DIR, &error_);
